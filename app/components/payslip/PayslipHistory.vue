...
class="mb-6" :pt="{
        ...
        <template #content>
            ...
            <div v-for="(slip, index) in historyItems" v-else :key="slip.payslip_id ?? index" @click="openSlip(slip)">
...
v-model:visible="showResolveDrawer" class="rounded-t-2xl   max-w-[768px]     "
        ...
        header="Payslip Details" :block-scroll="true" :style="{ maxHeight: '90vh' }">
        <template #header>
            <div class="flex items-center justify-between gap-2 w-full">
                <span class="font-bold">Payslip Details</span>

                <Button
label="Download Payslip" severity="contrast" variant="text"
                    :icon="downloading ? 'pi pi-spinner pi-spin' : 'pi pi-download'"
                    :disabled="downloading || !selectedPayslip" @click="downloadPayslip(selectedPayslip)" />
